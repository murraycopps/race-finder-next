...
export default function ShoeList({ shoes }: { shoes: Shoes}) {
    ...
    return(
        <div
	className={`flex flex-col items-center float-left gap-4 p-4 ml-2 mr-12 overflow-x-hidden text-white ${styles.width} rounded-2xl`}
>
	...
	<div className="flex flex-row items-center justify-center gap-2">
		...
		<p className={`w-64 text-4xl text-center ${(left || right) && styles['fade-change']}`}>
			{shoes[pastHalf ? (left ? (index > 0? index - 1 : shoes.length - 1) : (index < shoes.length - 1 ? index + 1 : 0)) : index].distance}
		</p>
