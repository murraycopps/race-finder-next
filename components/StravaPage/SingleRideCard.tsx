...
const SingleRideCard = ({ activity, close }: { activity: Run; close: () => void }) => (
  <div className="fixed inset-0 z-50 flex flex-col items-center justify-start gap-6 p-6 overflow-y-auto text-center rounded-lg shadow-lg lg:inset-10 bg-faded-base-500">
    ...
    <div className="grid w-full grid-cols-3 place-items-center">
      <div className="grid w-full grid-cols-3 col-span-2">
        <div className="flex flex-col items-center justify-start gap-2">
          <p className="text-lg">Max Elevation (m)</p>
          <p className="text-2xl">{activity.elev_high || "--"}</p>
        </div>
        <div className="flex flex-col items-center justify-start gap-2">
          <p className="text-lg">Min Elevation (m)</p>
          <p className="text-2xl">{activity.elev_low || "--"}</p>
        </div>
        <div className="flex flex-col items-center justify-start gap-2">
          <p className="text-lg">Elevation Gain (m)</p>
          <p className="text-2xl">{activity.total_elevation_gain || "--"}</p>
        </div>
      </div>
      <Link
